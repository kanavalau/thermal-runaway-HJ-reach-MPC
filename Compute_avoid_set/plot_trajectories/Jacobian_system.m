function df_dy = Jacobian_system(t,y,u,param)
Cp_r    = param(1);
Cp_c    = param(2);
rho_r   = param(3);
rho_c   = param(4);
...
df_dy(2,1) = nuA * k0 * exp(-Ea ./ (R * y(3,:)));
df_dy(2,2) = 0;
df_dy(2,3) = nuA * k0 * exp(-Ea ./ (R * y(3,:))) .* y(1,:).^nA .* (Ea ./ (R * y(3,:).^2));
df_dy(2,4) = 0;
...
df_dy(4,4) = - u/V_c - 1./(rho_c*V_c*Cp_c) .* (Uhtc .* A);
